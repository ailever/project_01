...
class GramSchmidt(nn.Module):
    ...
    def projection(self, a,b):
        return ((a*b).sum(dim=2, keepdim=True)/(b*b).sum(dim=2, keepdim=True)) * b

    def forward(self, X):
        for i in range(1, X.size(1)):
            X[:,i:i+1,:].sub_(self.projection(X[:,i:i+1,:], X[:,0:i,:]).sum(dim=1, keepdim=True).detach())
...
class AoA_Decoder_Core(nn.Module):
    ...
    def forward(self, xt, mean_feats, att_feats, p_att_feats, state, att_masks=None):
        """
        print(f' - xt : {xt.size()}')
        print(f' - mean_feats : {mean_feats.size()}')
        print(f' - att_feats : {att_feats.size()}')
        print(f' - p_att_feats : {p_att_feats.size()}')
        print(f' - state : {len(state)}\n{state[0].size(), state[1].size()}')
        #print(f' - att_mask : {att_mask.size()}')"""
