...
"""
# grnet
class gram_schmidt(nn.Module):

    '''
    def normalization(self, a):
        return (1/math.sqrt((a*a).sum())) * a
    '''

    def projection(self, a,b):
        return ((a*b).sum(dim=3, keepdim=True)/(b*b).sum(dim=3, keepdim=True)) * b
    ...
    def forward(self, X):
        ...
        for i in range(X.size(2)):
            if i == 0:
                ...
                Y[:,:,i:i+1,:] = X[:,:,i:i+1,:] - self.projection(X[:,:,i:i+1,:], Y[:,:,0:i,:]).sum(dim=2, keepdim=True)
        '''
        '''
        Y = X[:,:,0:1,:]
        for i in range(1, X.size(2)):
            Y_new = X[:,:,i:i+1,:] - self.projection(X[:,:,i:i+1,:], Y).sum(dim=2, keepdim=True).detach()
            Y = torch.cat([Y, Y_new], dim=2)
        Y = Y / Y.norm(dim=2, keepdim=True).detach()
        return Y
        '''
        for i in range(1, X.size(2)):
            X[:,:,i:i+1,:].sub_(self.projection(X[:,:,i:i+1,:], X[:,:,0:i,:]).sum(dim=2, keepdim=True).detach())
...
class Attention(nn.Module):
    def __init__(self, opt):
        ...
        if self.gsp != 0:
            self.orthonormalization = gram_schmidt()

    def forward(self, h, att_feats, p_att_feats, att_masks=None):
        if self.gsp != 0:
            att_feats = self.orthonormalization(att_feats)
            p_att_feats = self.orthonormalization(p_att_feats)
